...
def load_config(db):
    try:
        ...
        # On some fresh installs (or partially-initialized DBs), config rows may be missing,
        # which would otherwise crash the installer with a KeyError.
        required = [
            "bot_admin_id",
            "bot_token_admin",
            "bot_token_client",
        ...
        if missing:
            db.set_default_configs()
            config = db.select_str_config() or []
            for conf in config:
                configs[conf['key']] = conf['value']
            # Still ensure presence (keep as None if DB didn't provide it)
            for k in missing:
                configs.setdefault(k, None)
...
def set_config_variables(configs, server_url):
    # Be defensive: on a fresh install configs may not include keys yet.
