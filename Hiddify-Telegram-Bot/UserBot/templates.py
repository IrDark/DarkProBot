...
# Plan Info Template
def plan_info_template(plan, header="", final_price=None, discount_amount=0, discount_code=None):
    """Render plan info.

    Backward compatible: if final_price is None, it behaves like the original template.
    """
...
"""
    ...
    if final_price is not None and int(final_price) < price:
        msg += f"""{MESSAGES['PLAN_INFO_PRICE']} <s>{rial_to_toman(price)}</s> {MESSAGES['TOMAN']}
🎁 {MESSAGES.get('GIFT_CODE_APPLIED','Gift code applied')}: <code>{discount_code or '-'}</code>
➖ {MESSAGES.get('DISCOUNT_AMOUNT','Discount')}: {rial_to_toman(int(discount_amount))} {MESSAGES['TOMAN']}
✅ {MESSAGES.get('FINAL_PRICE','Final price')}: <b>{rial_to_toman(int(final_price))}</b> {MESSAGES['TOMAN']}
"""
    else:
        msg += f"""{MESSAGES['PLAN_INFO_PRICE']} {rial_to_toman(price)} {MESSAGES['TOMAN']}
"""
...
def plans_list_buy_header_template(server_title=None):
    """Header shown before listing plans in *buy* flow.

    Telegram clients often render inline keyboard buttons within the visual width of the
    message bubble. In renewal flow the preceding message is usually long (subscription
    details) which makes the bubble wider and buttons look full-width. This header makes
    the buy-flow bubble similarly wide, so plan buttons appear full-width too.
    """
    title = f"🖥 سرور انتخاب‌شده: {server_title}\n" if server_title else ""
    return (
        f"{title}"
