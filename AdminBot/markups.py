...
def users_bot_management_markup(value=None):
    ...
    # Referral management (settings) - placed above stats as requested
    markup.add(
        InlineKeyboardButton(
            KEY_MARKUP.get('REFERRAL_MANAGEMENT', '🎯 مدیریت دعوت از دوستان'),
            callback_data="users_bot_referral_settings:None",
        )
    )
    markup.add(InlineKeyboardButton(KEY_MARKUP.get('REFERRAL_STATS', '📊 آمار دعوت ها'), callback_data="users_bot_referral_stats:None"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['GIFT_CODE_MANAGEMENT'], callback_data=f"users_bot_gift_codes_menu:None"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['USERS_BOT_SETTINGS'], callback_data=f"users_bot_settings:None"))
    return markup


def users_bot_referral_settings_markup(settings: dict):
    """Referral system management menu (Admin)."""
    markup = InlineKeyboardMarkup()
    markup.row_width = 2

    enabled = bool(int(settings.get('referral_enabled') or 0) == 1)
    status = "✅" if enabled else "❌"

    markup.add(
        InlineKeyboardButton(
            f"{KEY_MARKUP.get('REFERRAL_ENABLED', 'فعال بودن سیستم دعوت')} | {status}",
            callback_data=f"users_bot_referral_toggle:{1 if enabled else 0}",
        )
    )

    markup.add(
        InlineKeyboardButton(KEY_MARKUP.get('REFERRAL_SET_REF_BONUS', '🎁 هدیه معرف'),
                             callback_data="users_bot_referral_set_referrer_bonus:None"),
        InlineKeyboardButton(KEY_MARKUP.get('REFERRAL_SET_NEW_BONUS', '🎁 هدیه کاربر جدید'),
                             callback_data="users_bot_referral_set_new_user_bonus:None"),
    )
    markup.add(
        InlineKeyboardButton(KEY_MARKUP.get('REFERRAL_SET_COMMISSION', '💸 درصد پورسانت خرید'),
                             callback_data="users_bot_referral_set_commission_percent:None"),
        InlineKeyboardButton(KEY_MARKUP.get('REFERRAL_EDIT_SHARE_TEXT', '✏️ ویرایش متن اشتراک گذاری'),
                             callback_data="users_bot_referral_edit_share_text:None"),
    )
...
# Gift Codes Management - Inline Keyboard Markup
def users_bot_gift_codes_menu_markup():
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(InlineKeyboardButton(KEY_MARKUP['GIFT_CODE_CREATE'], callback_data="users_bot_gift_code_create:None"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['GIFT_CODE_LIST'], callback_data="users_bot_gift_code_list:1"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data="users_bot_management_menu:None"))
    return markup

def users_bot_gift_codes_list_markup(codes, page=1):
    markup = InlineKeyboardMarkup(row_width=1)
    PER_PAGE = 10
    page = int(page)
    start = (page - 1) * PER_PAGE
    end = start + PER_PAGE
    sliced = codes[start:end]
    for c in sliced:
        status = "✅" if int(c.get("active",0))==1 else "⛔"
        dtype = "%" if c.get("type")=="percent" else MESSAGES.get("RIAL","Rial")
        title = f"{status} {c.get('code')} | {c.get('value')}{dtype} | {c.get('used_count',0)}/{c.get('max_uses',0) or '∞'}"
        markup.add(InlineKeyboardButton(title, callback_data=f"users_bot_gift_code_view:{c['id']}"))
    # pagination
    if len(codes) > end:
        markup.add(InlineKeyboardButton(KEY_MARKUP['NEXT_PAGE'], callback_data=f"users_bot_gift_code_list:{page+1}"))
    if page > 1:
        markup.add(InlineKeyboardButton(KEY_MARKUP['PREV_PAGE'], callback_data=f"users_bot_gift_code_list:{page-1}"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data="users_bot_gift_codes_menu:None"))
    return markup

def users_bot_gift_code_detail_markup(code_id: int, active: int):
    markup = InlineKeyboardMarkup(row_width=1)
    toggle_to = 0 if int(active)==1 else 1
    markup.add(InlineKeyboardButton(KEY_MARKUP['GIFT_CODE_TOGGLE'], callback_data=f"users_bot_gift_code_toggle:{code_id}|{toggle_to}"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['GIFT_CODE_DELETE'], callback_data=f"users_bot_gift_code_delete:{code_id}"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data="users_bot_gift_code_list:1"))
    return markup

def users_bot_gift_code_delete_markup(code_id: int):
    """Confirmation markup for deleting a gift/discount code."""
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(InlineKeyboardButton(KEY_MARKUP['CONFIRM'], callback_data=f"users_bot_gift_code_confirm_delete:{code_id}"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['CANCEL'], callback_data=f"users_bot_gift_code_view:{code_id}"))
    return markup

def users_bot_gift_code_type_markup():
    markup = InlineKeyboardMarkup(row_width=2)
    markup.add(InlineKeyboardButton("Percent (%)", callback_data="users_bot_gift_code_type:percent"),
               InlineKeyboardButton("Fixed (Rial)", callback_data="users_bot_gift_code_type:fixed"))
    markup.add(InlineKeyboardButton(KEY_MARKUP['BACK'], callback_data="users_bot_gift_codes_menu:None"))
    return markup
...
def users_bot_management_settings_markup(settings):
    ...
    status_addons = "✅" if settings.get('addon_plans_enabled') else "❌"
    status_addons_buy = "✅" if settings.get('addon_plans_show_in_buy') else "❌"
    ...
    # Add-on plans feature flags
    markup.add(InlineKeyboardButton(f"{KEY_MARKUP['USERS_BOT_SETTINGS_ADDON_PLANS']} | {status_addons}",
                                    callback_data=f"users_bot_settings_addon_plans:{settings.get('addon_plans_enabled')}"),
               InlineKeyboardButton(f"{KEY_MARKUP['USERS_BOT_SETTINGS_ADDON_PLANS_BUY']} | {status_addons_buy}",
                                    callback_data=f"users_bot_settings_addon_plans_buy:{settings.get('addon_plans_show_in_buy')}"))
...
def plans_list_markup(plans, server_id, delete_mode=False):
    ...
    if not delete_mode:
        ...
        keys.append(InlineKeyboardButton(KEY_MARKUP['USERS_BOT_ADD_VOLUME_PLAN'], callback_data=f"users_bot_add_volume_plan:{server_id}"))
        keys.append(InlineKeyboardButton(KEY_MARKUP['USERS_BOT_ADD_TIME_PLAN'], callback_data=f"users_bot_add_time_plan:{server_id}"))
