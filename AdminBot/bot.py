...
# Temporary state for creating gift codes
# {admin_id: {code, type, value, max_uses, per_user_limit, min_price}}
gift_code_create_session = {}
...
# Temporary state for editing referral settings
referral_settings_session = {}
...
# -------------------- Referral Settings Management (Admin) --------------------
def _referral_settings_summary(settings: dict) -> str:
    enabled = (_to_int(settings.get('referral_enabled') or 0) == 1)
    ref_bonus_rial = _to_int(settings.get('referral_signup_bonus_referrer_amount') or settings.get('referral_signup_bonus_amount') or 0)
    new_bonus_rial = _to_int(settings.get('referral_signup_bonus_new_user_amount') or settings.get('referral_signup_bonus_amount') or 0)
    percent = _to_int(settings.get('referral_commission_percent') or 0)
    try:
        ref_bonus_t = utils.rial_to_toman(ref_bonus_rial)
    except Exception:
        ref_bonus_t = str(ref_bonus_rial // 10)
    try:
        new_bonus_t = utils.rial_to_toman(new_bonus_rial)
    except Exception:
        new_bonus_t = str(new_bonus_rial // 10)
    status = "✅ فعال" if enabled else "❌ غیرفعال"
    return (
        "🎯 مدیریت دعوت از دوستان\n\n"
        f"وضعیت سیستم: {status}\n"
        f"🎁 هدیه معرف: {ref_bonus_t} تومان\n"
        f"🎁 هدیه کاربر جدید: {new_bonus_t} تومان\n"
        f"💸 درصد پورسانت خرید: {percent}٪\n\n"
        "برای تغییر هر مورد، از دکمه‌های زیر استفاده کنید."
    )


def next_step_set_referrer_bonus(message: Message):
    if is_it_cancel(message):
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['ERROR_INVALID_NUMBER'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_set_referrer_bonus)
        return
    toman = int(message.text)
    rial = toman * 10
    USERS_DB.edit_int_config('referral_signup_bonus_referrer_amount', value=rial)
    settings = utils.all_configs_settings()
    bot.send_message(message.chat.id, "✅ مبلغ هدیه معرف بروزرسانی شد.", reply_markup=markups.main_menu_keyboard_markup())
    bot.send_message(message.chat.id, _referral_settings_summary(settings), reply_markup=markups.users_bot_referral_settings_markup(settings))


def next_step_set_new_user_bonus(message: Message):
    if is_it_cancel(message):
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['ERROR_INVALID_NUMBER'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_set_new_user_bonus)
        return
    toman = int(message.text)
    rial = toman * 10
    USERS_DB.edit_int_config('referral_signup_bonus_new_user_amount', value=rial)
    settings = utils.all_configs_settings()
    bot.send_message(message.chat.id, "✅ مبلغ هدیه کاربر جدید بروزرسانی شد.", reply_markup=markups.main_menu_keyboard_markup())
    bot.send_message(message.chat.id, _referral_settings_summary(settings), reply_markup=markups.users_bot_referral_settings_markup(settings))


def next_step_set_commission_percent(message: Message):
    if is_it_cancel(message):
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['ERROR_INVALID_NUMBER'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_set_commission_percent)
        return
    percent = int(message.text)
    if percent < 0 or percent > 100:
        bot.send_message(message.chat.id, "عدد درصد باید بین 0 تا 100 باشد.", reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(message, next_step_set_commission_percent)
        return
    USERS_DB.edit_int_config('referral_commission_percent', value=percent)
    settings = utils.all_configs_settings()
    bot.send_message(message.chat.id, "✅ درصد پورسانت بروزرسانی شد.", reply_markup=markups.main_menu_keyboard_markup())
    bot.send_message(message.chat.id, _referral_settings_summary(settings), reply_markup=markups.users_bot_referral_settings_markup(settings))


def next_step_set_referral_share_text(message: Message):
    if is_it_cancel(message):
        return
    # Preserve Telegram text formatting where possible (bold/italic/links, etc.)
    try:
        text = message_to_html(message)
    except Exception:
        text = message.text or ""
    USERS_DB.edit_str_config('referral_share_text', value=text)
    settings = utils.all_configs_settings()
    bot.send_message(message.chat.id, "✅ متن اشتراک گذاری بروزرسانی شد.", reply_markup=markups.main_menu_keyboard_markup())
    bot.send_message(message.chat.id, _referral_settings_summary(settings), reply_markup=markups.users_bot_referral_settings_markup(settings))




# -------------------- Gift Code Management (Users Bot) --------------------
def next_step_gift_code_admin_code(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    code = (message.text or "").strip().upper()
    if not re.fullmatch(r"[A-Z0-9_-]{3,32}", code):
        bot.send_message(message.chat.id, MESSAGES['GIFT_CODE_INVALID_INPUT'], reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(message, next_step_gift_code_admin_code)
        return
    gift_code_create_session[message.chat.id] = {"code": code}
    bot.send_message(message.chat.id, MESSAGES['ASK_GIFT_TYPE'], reply_markup=markups.users_bot_gift_code_type_markup())

def next_step_gift_code_admin_value(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['GIFT_CODE_INVALID_INPUT'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_gift_code_admin_value)
        return
    gift_code_create_session[message.chat.id]["value"] = int(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_GIFT_MAX_USES'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, next_step_gift_code_admin_max_uses)

def next_step_gift_code_admin_max_uses(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['GIFT_CODE_INVALID_INPUT'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_gift_code_admin_max_uses)
        return
    gift_code_create_session[message.chat.id]["max_uses"] = int(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_GIFT_PER_USER'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, next_step_gift_code_admin_per_user)

def next_step_gift_code_admin_per_user(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['GIFT_CODE_INVALID_INPUT'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_gift_code_admin_per_user)
        return
    gift_code_create_session[message.chat.id]["per_user_limit"] = int(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_GIFT_MIN_PRICE'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, next_step_gift_code_admin_min_price)

def next_step_gift_code_admin_min_price(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['GIFT_CODE_INVALID_INPUT'], markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, next_step_gift_code_admin_min_price)
        return
    gift_code_create_session[message.chat.id]["min_price"] = int(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_GIFT_EXPIRY_DAYS'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, next_step_gift_code_admin_expiry)

def next_step_gift_code_admin_expiry(message: Message):
    if is_it_cancel(message):
        gift_code_create_session.pop(message.chat.id, None)
        return
    if not is_it_digit(message, allow_float=False, response=MESSAGES['GIFT_CODE_INVALID_INPUT'], markup=markups.while_edit_user_markup()):
    ...
    sess = gift_code_create_session.get(message.chat.id) or {}
    end_at = None
    if days > 0:
        end_at = (datetime.datetime.now() + datetime.timedelta(days=days)).isoformat(timespec="seconds")

    ok = USERS_DB.add_discount_code(
        code=sess["code"],
        dtype=sess["type"],
        value=sess["value"],
        max_uses=sess.get("max_uses", 0),
        per_user_limit=sess.get("per_user_limit", 1),
        min_price=sess.get("min_price", 0),
        start_at=datetime.datetime.now().isoformat(timespec="seconds"),
        end_at=end_at,
        active=1,
    )
    gift_code_create_session.pop(message.chat.id, None)
    if ok:
        bot.send_message(message.chat.id, MESSAGES['GIFT_CODE_CREATED'], reply_markup=markups.main_menu_keyboard_markup())
    else:
...
# Add-on plan creation temp storage
addon_plan_data = {}
...
# -------------------- Add-on plans (Volume/Time) --------------------
def users_bot_add_volume_plan_usage(message: Message):
    if is_it_cancel(message):
        addon_plan_data.clear()
        return
    if not is_it_digit(message, markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, users_bot_add_volume_plan_usage)
        return
    addon_plan_data['usage'] = int(message.text)
    addon_plan_data['days'] = 0
    bot.send_message(message.chat.id, MESSAGES['ASK_ADDON_PRICE_TOMAN'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, users_bot_add_addon_plan_price)


def users_bot_add_time_plan_days(message: Message):
    if is_it_cancel(message):
        addon_plan_data.clear()
        return
    if not is_it_digit(message, markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, users_bot_add_time_plan_days)
        return
    addon_plan_data['usage'] = 0
    addon_plan_data['days'] = int(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_ADDON_PRICE_TOMAN'], reply_markup=markups.while_edit_user_markup())
    bot.register_next_step_handler(message, users_bot_add_addon_plan_price)


def users_bot_add_addon_plan_price(message: Message):
    if is_it_cancel(message):
        addon_plan_data.clear()
        return
    if not is_it_digit(message, markup=markups.while_edit_user_markup()):
        bot.register_next_step_handler(message, users_bot_add_addon_plan_price)
        return
    addon_plan_data['price'] = utils.toman_to_rial(message.text)
    bot.send_message(message.chat.id, MESSAGES['ASK_ADDON_DESCRIPTION'], reply_markup=markups.while_edit_skip_user_markup())
    bot.register_next_step_handler(message, users_bot_add_addon_plan_desc)


def users_bot_add_addon_plan_desc(message: Message):
    if is_it_cancel(message):
        addon_plan_data.clear()
        return
    if message.text == KEY_MARKUP['SKIP']:
        addon_plan_data['description'] = None
    else:
        addon_plan_data['description'] = message.text

    msg_wait = bot.send_message(message.chat.id, MESSAGES['WAIT'], reply_markup=markups.while_edit_user_markup())
    ok = utils.users_bot_add_plan(
        addon_plan_data.get('usage', 0),
        addon_plan_data.get('days', 0),
        addon_plan_data.get('price', 0),
        addon_plan_data.get('server_id'),
        addon_plan_data.get('description')
    )
    bot.delete_message(message.chat.id, msg_wait.message_id)
    if not ok:
        bot.send_message(message.chat.id, MESSAGES['ERROR_UNKNOWN'], reply_markup=markups.main_menu_keyboard_markup())
        return
    bot.send_message(message.chat.id, MESSAGES['ADDON_PLAN_CREATED'], reply_markup=markups.main_menu_keyboard_markup())
    ...
    if plans:
        for plan in plans:
            if plan['status'] and plan['server_id'] == int(addon_plan_data['server_id']):
                plans_list.append(plan)
    plans_markup = markups.plans_list_markup(plans_list, addon_plan_data['server_id'])
    addon_plan_data.clear()
    bot.send_message(message.chat.id, MESSAGES['USERS_BOT_PLANS_LIST'], reply_markup=plans_markup)
...
def callback_query(call: CallbackQuery):
    ...
    # Use maxsplit=1 so values can safely contain ':' without breaking routing.
    data = (call.data or '').split(':', 1)
    if len(data) < 2:
        bot.answer_callback_query(call.id, MESSAGES.get('ERROR_UNKNOWN', 'Error'))
        return
    ...
    # ----------------------------------- Users Bot Management Callbacks -----------------------------------
    ...
    elif key == "users_bot_referral_settings":
        settings = utils.all_configs_settings()
        bot.edit_message_text(
            _referral_settings_summary(settings),
            call.message.chat.id,
            call.message.message_id,
            reply_markup=markups.users_bot_referral_settings_markup(settings),
        )

    elif key == "users_bot_referral_toggle":
        # value is current state (1/0); toggle it
        current = int(value) if str(value).isdigit() else 0
        new_val = 0 if current == 1 else 1
        USERS_DB.edit_bool_config('referral_enabled', value=new_val)
        settings = utils.all_configs_settings()
        bot.edit_message_text(
            _referral_settings_summary(settings),
            call.message.chat.id,
    ...
    elif key == "users_bot_referral_set_referrer_bonus":
        msg = bot.send_message(call.message.chat.id, "مبلغ هدیه معرف را به تومان ارسال کنید:", reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_set_referrer_bonus)

    elif key == "users_bot_referral_set_new_user_bonus":
        msg = bot.send_message(call.message.chat.id, "مبلغ هدیه کاربر جدید را به تومان ارسال کنید:", reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_set_new_user_bonus)

    elif key == "users_bot_referral_set_commission_percent":
        msg = bot.send_message(call.message.chat.id, "درصد پورسانت خرید را (0 تا 100) ارسال کنید:", reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_set_commission_percent)

    elif key == "users_bot_referral_edit_share_text":
        msg = bot.send_message(call.message.chat.id, "متن اشتراک گذاری جدید را ارسال کنید:", reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_set_referral_share_text)

    elif key == "users_bot_referral_stats":
        rows = USERS_DB.select_top_referrers_stats(limit=20)
        if not rows:
            bot.send_message(call.message.chat.id, "هیچ آماری برای دعوت از دوستان ثبت نشده است.")
            return
        ...
        lines = ["📊 آمار دعوت ها (۲۰ معرف اول)", ""]
        for i, r in enumerate(rows, start=1):
            tid = r.get('telegram_id')
            name = r.get('full_name') or r.get('username') or str(tid)
            referred_count = int(r.get('referred_count') or 0)
            total = _toman_str(int(r.get('total') or 0))
            signup_total = _toman_str(int(r.get('signup_total') or 0))
            comm_total = _toman_str(int(r.get('commission_total') or 0))
            lines.append(
                f"{i}. {name}\n"
                f"   👥 زیرمجموعه‌ها: {referred_count} | 🎁 هدیه: {signup_total} | 💸 پورسانت: {comm_total} | مجموع: {total} تومان"
            )
    ...
    elif key == "users_bot_gift_codes_menu":
        bot.edit_message_text(MESSAGES['GIFT_CODE_MENU'], call.message.chat.id, call.message.message_id,
                              reply_markup=markups.users_bot_gift_codes_menu_markup())

    elif key == "users_bot_gift_code_create":
        gift_code_create_session[call.message.chat.id] = {}
        msg = bot.send_message(call.message.chat.id, MESSAGES['ASK_GIFT_CODE'], reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_gift_code_admin_code)

    elif key == "users_bot_gift_code_type":
        sess = gift_code_create_session.get(call.message.chat.id)
        if not sess:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        dtype = value
        if dtype not in ("percent", "fixed"):
            bot.send_message(call.message.chat.id, MESSAGES['GIFT_CODE_INVALID_INPUT'])
            return
        sess["type"] = dtype
        msg = bot.send_message(call.message.chat.id, MESSAGES['ASK_GIFT_VALUE'], reply_markup=markups.while_edit_user_markup())
        bot.register_next_step_handler(msg, next_step_gift_code_admin_value)

    elif key == "users_bot_gift_code_list":
        codes = USERS_DB.list_discount_codes(active_only=False) or []
        if not codes:
            bot.send_message(call.message.chat.id, MESSAGES['GIFT_CODE_LIST_EMPTY'])
            bot.edit_message_text(MESSAGES['GIFT_CODE_MENU'], call.message.chat.id, call.message.message_id,
                                  reply_markup=markups.users_bot_gift_codes_menu_markup())
            return
        # Guard against invalid page values or out-of-range pages
        try:
            page = int(value)
        except Exception:
            page = 1
        if page < 1:
            page = 1
        # If requested page is beyond available items, fallback to page 1
        if (page - 1) * 10 >= len(codes):
            page = 1
        bot.edit_message_text(MESSAGES['GIFT_CODE_MENU'], call.message.chat.id, call.message.message_id,
                              reply_markup=markups.users_bot_gift_codes_list_markup(codes, page=page))

    elif key == "users_bot_gift_code_view":
        dc = USERS_DB.find_discount_code(id=int(value))
        if not dc:
            bot.send_message(call.message.chat.id, MESSAGES['GIFT_CODE_LIST_EMPTY'])
            return
        dc = dc[0]
        dtype = "%" if dc.get("type") == "percent" else MESSAGES.get("RIAL","Rial")
        end_at = dc.get("end_at") or "-"
        msg = f"""🎁 <b>{dc.get('code')}</b>
Type: <code>{dc.get('type')}</code>
Value: <b>{dc.get('value')}{dtype}</b>
Active: <b>{'✅' if int(dc.get('active',0))==1 else '⛔'}</b>
Used: <b>{dc.get('used_count',0)}</b> / <b>{dc.get('max_uses',0) or '∞'}</b>
Per-user limit: <b>{dc.get('per_user_limit',1)}</b>
Min price: <b>{dc.get('min_price',0)}</b>
End at: <code>{end_at}</code>
"""
        bot.edit_message_text(msg, call.message.chat.id, call.message.message_id,
                              reply_markup=markups.users_bot_gift_code_detail_markup(dc['id'], dc.get('active',0)))

    elif key == "users_bot_gift_code_delete":
        # Ask for confirmation before deleting a gift code
        try:
            code_id = int(value)
        except Exception:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        dc = USERS_DB.find_discount_code(id=code_id)
        if not dc:
            bot.send_message(call.message.chat.id, MESSAGES['GIFT_CODE_LIST_EMPTY'])
            return
        dc = dc[0]
        bot.edit_message_text(
            f"{MESSAGES.get('GIFT_CODE_DELETE_QUESTION','Are you sure you want to delete this gift code?')}\n\n🎁 <b>{dc.get('code')}</b>",
            call.message.chat.id,
            call.message.message_id,
            reply_markup=markups.users_bot_gift_code_delete_markup(code_id)
        )

    elif key == "users_bot_gift_code_confirm_delete":
        try:
            code_id = int(value)
        except Exception:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        ok = USERS_DB.delete_discount_code(code_id)
        if not ok:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        bot.send_message(call.message.chat.id, MESSAGES.get('GIFT_CODE_DELETED', '✅ Gift code deleted.'))
        # Refresh list (page 1)
        codes = USERS_DB.list_discount_codes(active_only=False) or []
        bot.edit_message_text(MESSAGES['GIFT_CODE_MENU'], call.message.chat.id, call.message.message_id,
                              reply_markup=markups.users_bot_gift_codes_list_markup(codes, page=1) if codes else markups.users_bot_gift_codes_menu_markup())

    elif key == "users_bot_gift_code_toggle":
        # value format: "<id>|<active>"
        try:
            code_id_str, active_str = value.split("|")
            code_id = int(code_id_str)
            active = int(active_str)
        except Exception:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        ok = USERS_DB.toggle_discount_code(code_id, active)
        if not ok:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        # refresh view
...
"""
    ...
    elif key == "users_bot_add_volume_plan":
        bot.send_message(call.message.chat.id, MESSAGES['ASK_ADDON_VOLUME_GB'], reply_markup=markups.while_edit_user_markup())
        addon_plan_data.clear()
        addon_plan_data['server_id'] = int(value)
        bot.register_next_step_handler(call.message, users_bot_add_volume_plan_usage)

    elif key == "users_bot_add_time_plan":
        bot.send_message(call.message.chat.id, MESSAGES['ASK_ADDON_TIME_DAYS'], reply_markup=markups.while_edit_user_markup())
        addon_plan_data.clear()
        addon_plan_data['server_id'] = int(value)
        bot.register_next_step_handler(call.message, users_bot_add_time_plan_days)
    ...
    elif key == "users_bot_settings_addon_plans":
        # toggle addon_plans_enabled
        if value == "1":
            ok = USERS_DB.edit_bool_config("addon_plans_enabled", value=False)
        else:
            ok = USERS_DB.edit_bool_config("addon_plans_enabled", value=True)
        if not ok:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        settings = utils.all_configs_settings()
    ...
    elif key == "users_bot_settings_addon_plans_buy":
        # toggle addon_plans_show_in_buy
        if value == "1":
            ok = USERS_DB.edit_bool_config("addon_plans_show_in_buy", value=False)
        else:
            ok = USERS_DB.edit_bool_config("addon_plans_show_in_buy", value=True)
        if not ok:
            bot.send_message(call.message.chat.id, MESSAGES['ERROR_UNKNOWN'])
            return
        settings = utils.all_configs_settings()
    ...
    # Payment - Confirm Payment Callback
        ...
        if payment_status:
            ...
            if not wallet_status:
                ...
                return
            bot.delete_message(call.message.chat.id, call.message.message_id)
            user_telegram_id = int(payment_info['telegram_id'])
            user_bot.send_message(
                user_telegram_id,
                f"{MESSAGES['WALLET_PAYMENT_CONFIRMED']}\n{MESSAGES['ORDER_ID']} {payment_id}"
            )

            # After wallet top-up, if the user had a pending checkout (buy/renewal), re-show the invoice.
            pending = USERS_DB.get_pending_invoice(user_telegram_id)
            if pending:
                try:
                    from UserBot.templates import plan_info_template
                    from UserBot.markups import confirm_buy_plan_markup

                    plan_rows = USERS_DB.find_plan(id=int(pending.get('plan_id')))
                    if plan_rows:
                        plan = plan_rows[0]
                        renewal = str(pending.get('kind')) == 'renewal'
                        uuid = pending.get('uuid') if renewal else None

                        if pending.get('final_price') is not None:
                            text = plan_info_template(
                                plan,
                                final_price=int(pending.get('final_price')),
                        ...
                        user_bot.send_message(
                            user_telegram_id,
                            text,
                            reply_markup=confirm_buy_plan_markup(
                                int(pending.get('plan_id')),
                                renewal=renewal,
                                uuid=uuid,
                    ...
                    logging.error(f"Failed to re-show pending invoice for {user_telegram_id}: {e}")
            bot.send_message(call.message.chat.id,
